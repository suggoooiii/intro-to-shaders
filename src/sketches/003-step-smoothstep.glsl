...
vec3 palette(in float t, in vec3 a, in vec3 b, in vec3 c, in vec3 d) {
  return a - b * cos(uTime + 6.5 * (c * t - d));
}
void main() {
  vec3 color = vec3(0.);
  float result = 0.;
  ...
  float sizeChange = sin(uTime) * 0.002;
...
//
  ...
  color = palette(result2 * 1. + .5, vec3(0.5, 0.5, 0.5), vec3(0.5, 0.5, 0.5), vec3(1.0, 1.5, 0.0), vec3(0.5, 0.20, 0.25));
